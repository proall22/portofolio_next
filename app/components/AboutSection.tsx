...
const timelineItems = [
  ...
  },
  {
    ...
    description: "Mastered cloud architecture and deployment",
    icon: Award,
  },
  {
    year: "2019",
            ...
            <div className="flex mb-8">
                ...
                {index !== timelineItems.length - 1 && (
                  <motion.div
                    className={`w-0.5 h-full ${
                      theme === "dark" ? "bg-teal-400" : "bg-gradient-to-b from-teal-400 to-emerald-500"
                    } mt-2`}
                    initial={{ height: 0 }}
                    animate={{ height: "100%" }}
              ...
              <motion.div
                ...
                whileHover={{
                  y: -5,
                  boxShadow:
                    theme === "dark" ? "0 10px 30px -15px rgba(0, 0, 0, 0.5)" : "0 10px 30px -15px rgba(0, 0, 0, 0.2)",
                }}
                transition={{
                  type: "spring",
                  stiffness: 300,
              ...
              >
                <h3 className={`text-xl font-semibold ${theme === "dark" ? "" : "text-emerald-600"}`}>{item.title}</h3>
                <p className={theme === "dark" ? "text-gray-400" : "text-gray-500"}>{item.year}</p>
                <p className={`mt-2 ${theme === "dark" ? "" : "text-gray-700"}`}>{item.description}</p>
              </motion.div>
            </div>
          </ScrollAnimationWrapper>
        ))}
